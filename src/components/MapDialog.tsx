...
export function MapDialog({ trip, sites, onClose }: { trip: Trip; sites: Site[]; onClose: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([listWaypoints(trip.id), listRouteTracks(trip.id), loadRecording(trip.id)]).then(([saved, savedRoutes, savedRecording]) => {
      if (active) { setWaypoints(saved); setRoutes(savedRoutes); setRecording(savedRecording); }
    });
    return () => { active = false; };
  }, [trip.id]);
  ...
  useEffect(() => {
    const center = markers[0] ?? visibleRoutes[0]?.points[0];
