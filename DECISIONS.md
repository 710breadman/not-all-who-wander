...
Optional accounts, cloud sync, and live shared trips use Firebase Authentication and Cloud Firestore. The approved account methods are email/password and Google sign-in. The existing IndexedDB application remains the offline-first source of immediate UI behavior. The first approved cloud scope is master inventory only; broader scopes and live sharing must wait for explicit privacy review plus roles/invite decisions. Conflicts use a deterministic revision/timestamp rule and retain the losing local copy. Local people profiles can store an ordinary email or Gmail address without enabling a cloud account.
